...
public class MainPanel extends Toolbox
{
	...
	public MainPanel()
	{
		...
		jtp.addTab("Add Shifts", new AddShiftPanel(this));
	...
	/**
	 * Removes employees from the list and replaces them in the order that the user
	 * has set.
	 * 
	 * @param listModel
	 */
	...
	public void removeEmployee(String fullName)
	{
		...
		if (emp == null)
		{
			JOptionPane.showMessageDialog(null, "Unable to find employee " + fullName, "Error", JOptionPane.INFORMATION_MESSAGE);
		}
		else
		{
			if(employees.size() == 0)
	...
	private void addExtraTestEmployees()
	{
		...
		{
			availabilityAll.add(new int[]
			{ 10, 23 });
		}
	
		ArrayList<int[]> availabilityWeekends = new ArrayList<int[]>();
		availabilityWeekends.add(new int[]
		{ 10, 23 });
		availabilityWeekends.add(new int[]
		{ -1, -1 });
		availabilityWeekends.add(new int[]
		...
		addEmployee(new Employee("John", "Doe", true, true, false, 39, availabilityAll), employeePanel.getEmployees());
