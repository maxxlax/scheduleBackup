...
public class EmployeePanel extends JPanel
{
	...
	private JLabel firstName, lastName, maxNumHoursLabel, avLab1, avLab2, avLab3, startEnd, sun, mon, tue, wed, thur,
			fri, sat;
	...
	public EmployeePanel(MainPanel mainPanel)
	{
		...
		employees.addActionListener(new EmployeeListener());
		add(employees);
		...
		public void actionPerformed(ActionEvent ae)
		{
			if (ae.getSource() == addEmployee)
			{
				...
				if (checkEmployee())
				{
					...
					{
						mainPanel.editEmployee(firstNameArea.getText(), lastNameArea.getText(), inshop.isSelected(),
								driver.isSelected(), canDouble.isSelected(),
								Integer.parseInt(maxNumHoursArea.getText()), getAvailability());
			...
			{
				setTextAreasColor(Color.WHITE);
				Employee emp = (Employee) employees.getSelectedItem();
	...
	private ArrayList<int[]> getAvailability()
	{
		ArrayList<int[]> av = new ArrayList<int[]>();
		for (int ii = 0; ii < availability.size(); ii += 2)
		{
			av.add(new int[]
			{ Integer.parseInt(availability.get(ii).getText()), Integer.parseInt(availability.get(ii + 1).getText()) });
		}
		return av;
	}
	...
	{
		employees.setSelectedItem(employee);
	}
	...
	public void nameChanged()
	{
		if (nameNotFound())
		{
			...
			// employees.addItem(newEmployee);
			// employees.setSelectedItem(newEmployee);
		}
		else
		{
			addEmployee.setText("Edit Employee");
			// employees.setSelectedItem(firstNameArea.getText() + " " +
