...
/**
 * A MultimediaApplication is a JApplication that delegates all calls to "transition" methods to a
 * MultimediaApp. The calls to the MultimediaApp object's "transition" methods will be made in the
 ...
 * @version 1.0
 */
public abstract class MultimediaApplication extends JApplication
    implements MultimediaRootPaneContainer
{
   ...
   *
   * @param args
   *          The command-line aguments
   * @param app
   *          The MultimediaApp to delegate to
   * @param width
   *          The width (in pixels) of the main window
   * @param height
   *          The width (in pixels) of the main window
   */
  public MultimediaApplication(String[] args, MultimediaApp app, int width, int height)
  {
    super(width, height);
   ...
   *
   * @param name
   *          The name/index of the parameter
   */
  ...
  /**
   * This method is called when the main window is first made visible and then each time it is
   * de-iconified.
   *
   * The default implementation does nothing.
   */
  ...
  public void exit()
  {
    ...
    if (response == JOptionPane.YES_OPTION)
    {
      if (app.saveShit())
      {
        if (JOptionPane.showConfirmDialog(mainWindow, "Successfully Saved, Close?", "Saved",
