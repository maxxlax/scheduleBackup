...
public class Employee implements Comparable<Employee>
{
  ...
  {
    String str = fullName + "|" + isInshop + "|" + isDriver + "|" + canDouble + "|" + currentHours
        + "|" + maxNumHours;
  ...
  public int setAvailability(Day day, int start, int end)
  {
    ...
    {
      case Sunday:
        availability.set(0, new int[] {start, end});
        break;
      case Monday:
        availability.set(1, new int[] {start, end});
        break;
      case Tuesday:
        availability.set(2, new int[] {start, end});
        break;
      case Wednesday:
        availability.set(3, new int[] {start, end});
        break;
      case Thursday:
        availability.set(4, new int[] {start, end});
        break;
      case Friday:
        availability.set(5, new int[] {start, end});
        break;
      case Saturday:
        availability.set(6, new int[] {start, end});
        break;
      default:
        ret = -1;
        break;
  ...
  {
    System.out.println("SDDD: " + shift.startTime + ">=" + getAvailability(shift.day)[0] + shift.endTime + "<=" + getAvailability(shift.day)[1]);
    // Within availability
    if (shift.startTime >= getAvailability(shift.day)[0]
        && shift.endTime <= getAvailability(shift.day)[1])
    {
      return true;
    }
    return false;
  ...
  public boolean wontExceedMax(Shift currentShift)
  {
    return currentHours + currentShift.getTotalTime() <= maxNumHours;
  }
  
  public boolean canWorkShift(Shift currentShift)
  {
    return isAvailable(currentShift) && can(currentShift.type) && wontExceedMax(currentShift);
  }
